...
- 3XX: 需要重定向, 浏览器直接跳转, 如302,304(缓存)
- 4XX: 客户端请求错误, 如:  403(资源存在,但是没有权限访问该资源), 404 
- 5XX: 服务器端错误, 如: 500

#### 区别ajax请求与一般HTTP请求

​	注意:ajax(async javascript and xml)只是发送http请求的一种技术

​			ajax请求仅存于浏览器端,但是浏览器端发送的请求不一定都是ajax(link,script,img)

- ajax请求是一种特别的http请求
- 对服务器端来说, 没有任何区别, 区别在浏览器端
  - 区别:HTTP请求不会报跨域错误,ajax会
- 浏览器端发请求: 只有XHR(ajax请求)或fetch发出, 其它所有的都是非ajax请求
- 浏览器端接收到响应
